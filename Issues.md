...
## v2.5.0 — 已修復（2026-03-18）

- [x] `getTargetForRange` 支援 2025 + 跨年區間目標計算（原本硬編碼 2026，選 2025 會 fallback 全年目標）
- [x] 明細表排序改用 `data-v` 原始數值（原本 parseFloat 格式化字串 `1,200k` → 得到 `1`）
- [x] resize 時 lazy init 桌機圖表（手機載入→拉寬視窗時補建 yearPred / officialMonthly）
- [x] 自動刷新後重建 client 篩選選項，保留仍有效的已選狀態


## v2.4.1 — 已修復（2026-03-18）

- [x] Client chips 顯示名稱去龍群後綴
- [x] KPIGrid fmtK 加千分位符號
